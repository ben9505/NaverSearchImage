...
public class RestAPI {
    ...
    public static void GetAPI(final Context context, String url, final RestListenner restListenner) {
        ...
        Ion.with(context)
                .load(url)
                .addHeader("X-Naver-Client-Id", context.getString(R.string.naver_clientId))
                .addHeader("X-Naver-Client-Secret", context.getString(R.string.naver_secret))
                .as(NaverImage.class)
                .withResponse()
                        ...
                        if (result != null) {
                            if (result.getHeaders().code() == 200 || result.getHeaders().code() == 201) {
                                if (result.getResult().getItems().size() > 0) {
                                    ...
                                    restListenner.onError(0, context.getString(R.string.error_empty));
                                }
                            } else {
                                restListenner.onError(result.getHeaders().code(), "");
                            }
                        } else {
                            restListenner.onError(0, context.getString(R.string.error_network));
                        }
                    }
                });
    }
